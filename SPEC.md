...
Request流体が指定されている場合の下限閾値は、

**15,000 fluid**

とする。

Request流体が指定されている場合の上限閾値は、
...
30 tickごとに、指定流体についてfluidboxの量を下限15,000、目標20,000、上限25,000のヒステリシスバッファとして扱う。

現在量が15,000未満の場合のみ、不足量を計算し、Dimensional Storageから20,000まで補充する。

現在量が15,000以上25,000未満の場合は、通常の配管バッファ範囲として扱い、補充も吸収もしない。

現在量が25,000へ到達した場合、20,000を超える分を実fluidboxから取り出し、Dimensional Storageへ格納する。

Factorioのfluid amountが浮動小数として扱われる可能性を考慮し、実装では容量到達を検出するためのごく小さい許容差を使用してよい。ただし20,000を少し超えただけで吸収してはならない。
